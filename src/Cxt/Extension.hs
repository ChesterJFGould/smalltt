...
binding (Cxt lvl env mask tbl typ ttyp mcxt ns frz) x i a k = do
  ...
  case x of
    BEmpty  -> do
      k (Cxt lvl' env' mask' tbl (M.insert lvl a typ) ttyp mcxt (NCons ns NEmpty) frz) v
    BSpan x -> do
      h   <- ST.hash tbl x
      old <- ST.insertWithHash x h (ST.Local lvl a) tbl
      res <- k (Cxt lvl' env' mask' tbl (M.insert lvl a typ) ttyp mcxt (NCons ns (NSpan x)) frz) v
...
defining (Cxt lvl env mask tbl typ ttyp mcxt ns frz) x a ~vt k = do
  when (lvl >= maxLocals) $ throw $ TooManyLocals
  h   <- ST.hash tbl x
  old <- ST.insertWithHash x h (ST.Local lvl a) tbl
  res <- k (Cxt (lvl + 1) (EDef env vt) mask tbl (M.insert lvl a typ) ttyp mcxt (NCons ns (NSpan x)) frz)
  ST.updateWithHash x h old tbl
  pure res
{-# inline defining #-}
...
inserting (Cxt lvl env mask tbl typ ttyp mcxt ns frz) x d k = do
  when (lvl >= maxLocals) $ throw $ TooManyLocals
  let v = VLocalVar lvl SId
  k (Cxt (lvl + 1) (EDef env v) (LS.insert lvl mask) tbl (M.insert lvl d typ) ttyp mcxt (NCons ns x) frz) v
{-# inline inserting #-}
