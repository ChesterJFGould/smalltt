...
module UnificationTyped (unifyTy, solve, unifySp) where
    ...
    case (t, t', ty) of
      ...
      (VUnfold h sp t, VUnfold h' sp' t') -> case cs of
        Rigid | eqUH h h' -> unifySp ms l frz Flex sp sp'
                                `catch` \_ -> unifyChk ms l frz Full (G topt t) (G topt' t')
              | otherwise -> unifyChk ms l frz Rigid (G topt t) (G topt' t')
...
doCloApp :: Closure -> Val -> Val
doCloApp = _

{-
unifySyn :: MetaCxt -> Lvl -> MetaVar -> ConvState -> G -> G -> IO ()
unifySyn ms l frz cs (G topt ftopt) (G topt' ftopt') = let
  go       = unifySyn ms l frz cs; {-# inline go #-}
  go' t t' = go (gjoin t) (gjoin t'); {-# inline go' #-}
  err      = throw (UnifyEx Conversion); {-# inline err #-}

  goBind t t' =
    let v = VLocalVar l SId
    in unifySyn ms (l + 1) frz cs (gjoin $! appCl' ms t v)
                               (gjoin $! appCl' ms t' v)
  {-# inline goBind #-}
  ...
  in do
    ...
    case (t, t') of
      ...
      (VUnfold h sp t, VUnfold h' sp' t') -> case cs of
        Rigid | eqUH h h' -> unifySp ms l frz Flex sp sp'
                                `catch` \_ -> unifySyn ms l frz Full (G topt t) (G topt' t')
              | otherwise -> unifySyn ms l frz Rigid (G topt t) (G topt' t')
        Flex  | eqUH h h' -> unifySp ms l frz Flex sp sp'
              | otherwise -> err
        _                 -> impossible
...
--   from hot code.
{-
-- Shouldn't need this since we do eta in unifyChk
...
{-
unifySpCold :: MetaCxt -> Lvl -> MetaVar -> ConvState -> Lvl -> Spine -> Val -> IO Val
unifySpCold = _
-}
{-
